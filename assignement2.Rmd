...
# read data
seeds <- read.csv("/Users/lilach/Documents/master/שנה א/סמסטר ב/למידת מכונה- סול/תרגילים/תרגיל 2/assignment-2/seeds.csv")
...
The aim: classify the seedr types.
...
Since we want to use KNN, we have to make sure the range of values of each parameter is similar. We'll have to normalize our data to make sure our classifier will work. 
...
Now, we will run the KNN algorithm and We will start with K of 12, the square root of 159 and also an odd number, reducing the change of a tie vote.

```{r}
seeds_test_pred <- knn(train = seeds_knn_train, test = seeds_knn_test, cl = seeds_train_types, k=12)
```
...
seeds_test_pred_z <- knn(train = seeds_train_z, test = seeds_test_z, cl = seeds_train_types_z, k=12)
