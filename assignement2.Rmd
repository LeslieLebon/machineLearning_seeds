...
Now, we wnat to evaluate our models.
...
The changes can either be random (guessing) or more precise, depending on performance metrics. We will use the area under the Receiver Operating Characteristic (ROC) curve (which demonstrates the trade-off between the sensitivity and and specificity), and overall classification accuracy.
...
all_probs_DT <- predict(seeds_model,seeds_test,type="prob")

# Print out all_probs
all_probs_DT

# Select second column of all_probs: probs
probs_DT <- all_probs_DT[,2]



# Make a prediction object: pred
pred_DT <- prediction(probs_DT,seeds_test$Types)
...
plot(perf_DT)
```
we will obtain a curve to interpret.

We will do the same with KNN
```{r}

# Predict probability values using the model: all_probs
all_probs_KNN <- predict(seeds_test_pred,test,type="prob")

# Print out all_probs
all_probs_KNN

# Select second column of all_probs: probs
probs_KNN<- all_probs_KNN[,2]



# Make a prediction object: pred
pred_KNN <- prediction(probs_KNN,seeds_test$Types)
...
plot(perf_KNN)
```
It will give us another curve but for KNN
And then we compare the 2 performances
...
pred_t <- prediction(probs_DT, seeds_test$Types)
pred_k <- prediction(probs_KNN, seeds_test$Types)

# Make the performance objects for both models: perf_t, perf_k
perf_t <- performance(pred_DT,"tpr","fpr")
perf_k <- performance(pred_KNN,"tpr","fpr")

# Draw the ROC lines using draw_roc_lines()
draw_roc_lines(perf_DT,perf_KNN)
```
It will give 2 curves one for KNN and one for DT.
```{r}
# Make the performance object of pred_t and pred_k: auc_t and auc_k
auc_t <- performance(pred_DT,"auc")
auc_k <- performance(pred_KNN,"auc")

# Print the AUC value
auc_t@y.values[[1]] # AUC result is 
auc_k@y.values[[1]] # AUC result is 
```
and then we can compare the 2 values obtained and decide which algorithm is the best to predict the seed types.
